...
export const Header = () => {
  ...
  const [hoveredImage, setHoveredImage] = useState(defaultImage); // Default image
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setVisible(moving < 350 ? true : position > moving);
      setPosition(moving);
      setMovingUp(moving > lastScrollY);
      setLastScrolly(moving);
    };
  ...
  return (
    <>
      ...
      ${movingUp ? "relative top-0 left-0" : "sticky top-0 left-0"}
      `}
      >
        <div
          className={` grid grid-cols-24 h-[110px] sm:h-[300px] border-2 ${borderColor}`}
        >
          {/* First Grid - 4 Vertical Parts */}
          <div
          ...
          >
            ...
            {/* CV, Contact, and Toggle - Only visible on mobile (sm and below) */}
          ...
          {/* Fourth Grid - Hidden on mobile */}
          <div
            className={`col-span-5 hidden md:flex flex-col border-r-2 ${borderColor}`}
          >
            {projects
              ?.filter((project) => project.featured) // Show only featured projects
              .map((project, index, filteredProjects) => (
                <Link
                  ...
                  onMouseEnter={() => {
                    setHoveredImage(project.icon.fields.file.url); // Set image on hover
                    setIsHovered(true); // Apply hover effect to third grid
                  }}
                  onMouseLeave={() => {
                    setHoveredImage(defaultImage); // Reset image on leave
                    setIsHovered(false); // Remove hover effect
                  }}
                >
          ...
          <div className="col-span-4 sm:col-span-1 flex items-center justify-center">
            {/* Visible only on medium (md) and larger screens */}
            ...
            {/* Visible only on small (sm) screens but hidden on extra small */}
      ...
      <div
        className={`w-full bg-red-500 z-550 ${
          movingUp ? "sticky top-[0rem]" : "absolute top-[300px]"
        }  `}
      >
        <SlideBar />
      </div>
