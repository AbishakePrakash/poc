...
export function Projects() {
  ...
  return (
    <section id="projects" className={`text-center border-b-2 border-t-2 w-full h-full`}>
      ...
      <div className={`grid grid-cols-12 w-full h-full`}>
        ...
        <div className="col-span-6 md:col-span-8 flex items-center sm:pl-10 py-10 justify-start  border-r-2 h-full">
          <p className="typography-ps1  text-left w-full pl-5">Selected Works</p>
        </div>
        ...
        <div className="col-span-6 md:col-span-4  flex items-center justify-center cursor-pointer" >
          <Link href="/projects" className="flex items-center space-x-2">
            ...
            <svg width="20" height="20" viewBox="0 0 15 27" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M15 13.5L-1.18272e-06 26.0574L-8.49151e-08 0.94263L15 13.5Z" fill="currentColor" />
            </svg>
          </Link>
        </div>
      </div>
      ...
      ${index % 2 !== 1 ? "sm:border-r-2" : ""}`}
          >
            ...
            <h3 className="typography-ps3 self-start text-left w-full">{project.projectTitle}</h3>
          </Link>
        ))}
      </div>
